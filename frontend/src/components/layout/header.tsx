...
export function AppHeader({ onMenuToggle }: HeaderProps) {
  ...
  useEffect(() => {
    // Get the hour in the local browser timezone
  ...
  return (
    <>
      <header className="sticky top-0 z-50 flex items-center justify-between px-4 md:px-6 h-16 w-full bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-100 dark:border-slate-800 shadow-[0px_4px_12px_rgba(26,43,60,0.05)]">
        ...
        {/* Right section: Actions (including mobile + Tambah Transaksi button) */}
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={cn(
              "w-9 h-9 flex items-center justify-center rounded-full transition-colors active:scale-95 duration-150",
              isSubscribed ? "text-primary" : "text-foreground hover:bg-muted"
            )}
            title={isSubscribed ? "Matikan Notifikasi" : "Nyalakan Notifikasi"}
          >
            <span className="material-symbols-outlined text-xl" style={{ fontVariationSettings: `'FILL' ${isSubscribed ? 1 : 0}` }}>
          ...
          <button
            ...
            className="w-9 h-9 flex items-center justify-center rounded-full text-foreground hover:bg-muted transition-colors active:scale-95 duration-150"
          >
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="w-9 h-9 flex items-center justify-center rounded-full text-foreground hover:bg-muted transition-colors active:scale-95 duration-150 outline-none">
