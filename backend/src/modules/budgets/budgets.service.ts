...
export interface EnrichedBudget extends Omit<Budget, 'amount' | 'spent'> {
  amount: number;
  spent: number;
  remaining: number;
  percentage: number;
...
export class BudgetsService {
  ...
  async create(userId: string, dto: CreateBudgetDto): Promise<EnrichedBudget> {
  ...
  async findAll(userId: string): Promise<EnrichedBudget[]> {
  ...
  async findById(userId: string, id: string): Promise<EnrichedBudget> {
  ...
  async getSummary(userId: string, date: Date = new Date()) {
    ...
    const overBudgetCount = activeBudgets.filter(
      (b) => b.spent > Number(b.amount),
    ).length;
    const nearLimitCount = activeBudgets.filter((b) => {
      const pct = (b.spent / Number(b.amount)) * 100;
  ...
  private async enrichBudgetWithSpent(
    userId: string,
    budget: Budget,
  ): Promise<EnrichedBudget> {
